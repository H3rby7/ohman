...
func GetTodoListHandler(c *gin.Context) {
	c.JSON(http.StatusOK, model.Get())
}
...
func AddTodoHandler(c *gin.Context) {
	...
	c.JSON(statusCode, gin.H{"id": model.Add(todoItem.Message)})
}

// DeleteTodoHandler will delete a specified todo based on user http input
func DeleteTodoHandler(c *gin.Context) {
	todoID := c.Param("id")
	if err := model.Delete(todoID); err != nil {
...
func CompleteTodoHandler(c *gin.Context) {
	...
	if model.Complete(todoItem.ID) != nil {
		c.JSON(http.StatusInternalServerError, err)
		return
	}
	c.JSON(http.StatusOK, "")
...
func convertHTTPBodyToTodo(httpBody io.ReadCloser) (model.Todo, int, error) {
	body, err := ioutil.ReadAll(httpBody)
	if err != nil {
		return model.Todo{}, http.StatusInternalServerError, err
	}
	defer httpBody.Close()
	return convertJSONBodyToTodo(body)
}

func convertJSONBodyToTodo(jsonBody []byte) (model.Todo, int, error) {
	var todoItem model.Todo
	err := json.Unmarshal(jsonBody, &todoItem)
	if err != nil {
		return model.Todo{}, http.StatusBadRequest, err
	}
